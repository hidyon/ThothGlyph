...
**ラベルを書いたブロック数式だけ**を、文書順に1から採番する。ラベルの書き方は
2通りあり、**どちらも同じ採番の列に載る**。

| 記法 | ラベル | 参照 | 由来 |
|---|---|---|---|
| 新（0083） | 閉じ `$$` の後ろの `{#eq-名前}` | `@eq-名前` | Quartoの綴り |
| 旧（0044） | LaTeXの `\tag{名前}` | `[(1)](#eq-名前)` | 標準の道具を借りた形 |

`{#eq-名前}` は**ブロック数式と同じ1つの塊として退避する**（`MATH_PATTERN` に
続きとして書いてある）ので、本文に文字として残らない。`@eq-名前` は旧記法の
参照と同じ経路で退避し、差し戻しのときに番号を引く。**ラベルがASCII小文字だけ**
なのは、`@eq-密度より` のように日本語が続くとラベルの終わりが判定できないため。
両方の記法が同じ式にあるときは `{#eq-…}` を採る。

以下は旧記法を入れた0044からの説明で、描き方は新記法でも同じ。
