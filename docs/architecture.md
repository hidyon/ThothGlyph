...
### 式の番号と参照（[0044](specs/0044-equation-numbers.md)・[0083](specs/0083-crossref-dialect.md)）

**ラベル（`{#eq-名前}`）を書いたブロック数式だけ**を、文書順に1から採番する。
本文からの参照は `@eq-名前`。綴りはQuartoに合わせてある（0083）。

`{#eq-名前}` は**ブロック数式と同じ1つの塊として退避する**（`MATH_PATTERN` に
続きとして書いてある）ので、本文に文字として残らない。`@eq-名前` は数式と同じ
走査で退避し、差し戻しのときに番号を引く。**ラベルがASCII小文字だけ**なのは、
`@eq-密度より` のように日本語が続くとラベルの終わりが判定できないため。

**0044の記法（`\tag{名前}` と `[(1)](#eq-名前)`）は0085で落とした。**
`\tag` はもう見ないので、**LaTeXを加工せずKaTeXへ渡す**（KaTeXが自分で描く）。
`[(1)](#eq-名前)` もただのリンクになる。`renderMarkdown` から
`TAG_PATTERN`・`taggedFormula`・`REF_PATTERN` が消えているのはこのため。

以下は0044からの説明で、描き方は新記法でも同じ。
