...
**ファイルの入出力**のうち0012（読み込み）は完了した。置き場所は
**ソースペインの見出し行**で、見出し行の縦paddingを0＋`min-height: 30px` にして
ボタンを足しても高さが変わらない形にしてある。
[0034](0034-file-save.md)（書き出し）は**同じ行にもう1つ足すだけ**でよく、
幅360pxでも見出し行はまだ空いている（「ソース」37px＋ボタン106px／360px）。
