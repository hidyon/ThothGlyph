...
0024でパレットのラベルを `SymbolPalette` の外（`engine.renderLatex`）から受け取る形に
したので、**0032（狭い画面でパレットが3割）でラベルの描き方を変えるなら、
ここが手がかりになる**。0024のスコープ外に置いた「起動時にラベル122件を
KaTeXで一気に描くCPUコスト」も、0032と同じ場所の話。
