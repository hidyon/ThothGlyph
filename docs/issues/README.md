...
| [0055](0055-greek-uppercase-complete.md) | ギリシャ大文字が11件しかなく24件そろっていない | open | — |
...
（記号7グループ89件・公式12分類60件）ので、パレットに何かを足すissueでは
...
- **0055（大文字24件）は独立**。データを1タブに13行足すだけで、
  0053・0054のどちらとも競合しない。ただし
  [0029](0029-greek-complete.md) が意図して除いた13件を**入れ直す**ものなので、
  仕様に「なぜ判断を変えるか」と「見分けがつかない問題への答え」を書くこと。
  README・単体テストの件数（記号7グループ89件 → 102件）も直す。
