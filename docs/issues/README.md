...
| [0026](0026-typescript-strict.md) | TypeScriptがstrictでない | open | — |
...
**安いうちにやると得なもの**: 0026（strict）は実測でエラー0件なので、いま入れれば
1行で済む。状態が増えてからでは直す量が増える。0023（履歴からの復旧）は保存形式を
変えるので、0004（複数文書）より先か同時がよい。
