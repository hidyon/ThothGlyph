...
section('personal-snippets', '自分用スニペット（0087）', async () => {
  await page.setViewportSize({ width: 1440, height: 900 })
  await appReady()
  await editor().fill('before x+1 after')
  await editor().evaluate((element) => { element.focus(); element.setSelectionRange(7, 10) })
  await page.getByRole('tab', { name: '自分用' }).click()
  await page.getByRole('button', { name: '選択範囲を登録' }).click()
  await page.getByLabel('名前').fill('平方根')
  await page.getByLabel('内容').fill('\\sqrt{%CURSOR%}')
  await page.getByRole('button', { name: '登録', exact: true }).click()
  check('選択した内容を名前付きで登録できる', await page.getByRole('button', { name: '平方根', exact: true }).isVisible())
  await editor().evaluate((element) => { element.focus(); element.setSelectionRange(7, 10) })
  await page.getByRole('button', { name: '平方根', exact: true }).click()
  check('自分用スニペットが選択範囲を囲んで挿入される', (await editor().inputValue()) === 'before \\sqrt{x+1} after')
  await page.getByRole('button', { name: '編集', exact: true }).click()
  await page.getByLabel('名前').fill('平方根（編集済み）')
  await page.getByRole('button', { name: '更新', exact: true }).click()
  check('名前と本文を編集できる', await page.getByRole('button', { name: '平方根（編集済み）', exact: true }).isVisible())
  await page.setViewportSize({ width: 360, height: 780 })
  await page.screenshot({ path: OUT + '/personal-snippets-narrow.png', fullPage: true })
  const narrow = await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)
  check('幅360pxで自分用タブに横スクロールが出ない', narrow)
  await page.reload()
  await appReady()
  await page.getByRole('tab', { name: '自分用' }).click()
  check('リロード後も自分用スニペットが残る', await page.getByRole('button', { name: '平方根（編集済み）', exact: true }).isVisible())
  page.once('dialog', (dialog) => dialog.accept())
  await page.getByRole('button', { name: '削除', exact: true }).click()
  check('確認後に削除される', (await page.getByRole('button', { name: '平方根（編集済み）', exact: true }).count()) === 0)
  await page.setViewportSize({ width: 1440, height: 900 })
  await page.screenshot({ path: OUT + '/personal-snippets.png', fullPage: true })
  await resetState()
})
