...
**cuneus**（クネウス）は、数式入りのMarkdownを書くためのWebエディタ。
...
| **記号パレット** | 記号7グループ102個。ボタンを押すとカーソル位置に挿入される |
...
**cuneus**（クネウス）はラテン語で「楔（くさび）」。楔形文字 *cuneiform* の
語源にあたる語で、cuneus（楔）+ forma（形）からできている。

楔形文字は、葦の茎の先を粘土板に押し当てて刻む。だから cuneus は**刻まれる形**の
名であり、同時に**刻むための道具**の名でもある。

そして、人類が最初に数式を書きつけた場所がその粘土板だった。紀元前1800年頃の
一枚 **YBC 7289** には、正方形とその対角線、そして √2 の近似値が60進法で
刻まれている（1;24,51,10 ＝ 1.41421296…、真の値との誤差は100万分の1以下）。
別の一枚 **Plimpton 322** にはピタゴラス数の表が並ぶ。ピタゴラスより
1000年以上前のものだ。

記号をパレットから選んで文書に打ち込む——この道具でいちばんよく使う操作は、
楔を粘土に打ち込む動作にそのまま重なる。書いたものをその場に溜め込まず、
外へ持ち出して使うところも、運ぶために作られた粘土板に似ている。

表記は小文字のみの `cuneus`。文頭でも大文字にしない。
