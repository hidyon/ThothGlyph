...
**画面の寸法は個々のissueで測らない**（2026-09-22の方針）。機能をいくつか作ってから、
調整のissueでまとめて合わせる。幅に関わる検証も既定では流さない
...
2. **仕様を書く** — `docs/specs/NNNN-slug.md` に書く。テンプレートは
   ...
   UIの方針が承認後に2回変わったら、実装を続ける前に**調整仕様**として、見出し・操作の置き場所・
   採用／削除する画像・確認する画面幅を短く追記し、現在の最終形を固定する。
...
4. **検証する** — 受け入れ基準を**実際に動かして**確認する。型が通ることは
   ...
   操作をメニューへ移す変更では、メニューを開いて項目を選ぶ検証へ既存チェックも同じコミットで更新する。
...
- 生成した画像は採用を決めた時点で `README`・`public/`・`dist-file/` の配置先を決める。
  使わない試作画像は次の生成前に片付け、配布物へ混ぜない。
